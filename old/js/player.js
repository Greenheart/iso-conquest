...
class Player {
    ...
    conquer(area, shouldTryConqueringAdjacent = false) {
        ...
        if (area.owner !== null && area.owner !== this) {
            if (area.bonus) {
                area.owner.bonusAreas = area.owner.bonusAreas.filter((bonus) =>
                    Area.removeSpecific(bonus, area),
                )
            }
            area.owner.score -= area.value
            area.owner.areas = area.owner.areas.filter((areaToTest) =>
                Area.removeSpecific(areaToTest, area),
            )
        }
        area.owner = this
        ...
        if (shouldTryConqueringAdjacent === true) {
            const adjacentHostileAreas = area.adjacentAreas[1].filter((a) =>
    ...
    conquerBySacrifice(area) {
        ...
        this.areas = this.areas.filter((areaToTest) =>
            Area.removeSpecific(areaToTest, this.currentlySelectedArea),
        )
        if (this.currentlySelectedArea.bonus) {
            this.bonusAreas = this.bonusAreas.filter((bonus) =>
                Area.removeSpecific(bonus, this.currentlySelectedArea),
            )
        }
        this.currentlySelectedArea.owner = null
        this.currentlySelectedArea.viewComponent.classList.remove(
            'player' + this.id,
        )
    ...
    getOwnAreas() {
        return this.game.currentLevel.areas.filter(
            (area) => area.owner === this,
        )
    }
